...
(3,495) and **`curiosity`** (14,082). The default view is the `relevant` seam minus three
hide-flags — regional, paradigm variants and obsolete spellings — about 2,800 words. Each
of those, plus diminutives, is one three-state control in the UI: „fără / cu / doar". See
`CLAUDE.md` → Seams.
...
Every row also carries `quality_score`, `seam` (`relevant` / `curiosity`) and the class
flags `regional_only`, `variant_like`, `archaic_spelling`, `diminutive_like`. See
`CLAUDE.md` → Seams for how the score and the flags divide the work.
